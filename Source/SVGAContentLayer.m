...
- (void)stepToFrame:(NSInteger)frame {
    ...
    // ✅ 关键修改：在第一次显示时才解码图片
    if (self.bitmap != nil && self.bitmapLayer != nil && self.bitmapLayer.contents == nil) {
        // 第一次访问 CGImage 时触发 SVGAImage 的按需解码
        self.bitmapLayer.contents = (__bridge id _Nullable)([self.bitmap CGImage]);
        // 此时才触发 SVGAImage 解码
    }
